...
class AiContactSafeModelAttachments extends AiContactSafeModelDefault {
	...
	function getAttachments() {
		...
		foreach($recorded_files as $recorded_file) {
			...
			$file = $path_upload.DIRECTORY_SEPARATOR.$recorded_file->name;
	...
	function delete() {
		...
		foreach($cid as $id) {
			// get the file name
			$file_name = JFactory::getApplication()->input->get('file_'.$id, '');
	...
	function downloadFile() {
		...
		$file_name = JFactory::getApplication()->input->get('file', '');
		// get the path to attachments upload
		$upload_folder = str_replace('\\',DIRECTORY_SEPARATOR,$this->_config_values['upload_attachments']);
		$upload_folder = str_replace('/',DIRECTORY_SEPARATOR,$upload_folder);
		$upload_folder = str_replace('&#92;',DIRECTORY_SEPARATOR,$upload_folder);
