...
class AiContactSafeModelProfiles extends AiContactSafeModelDefault {
	...
	function checkBeforeWrite($postData) {
		...
		switch( true ) {
			case $this->_task == 'save_contact' or $this->_task == 'apply_contact' :
				$postData['contact_info'] = JFactory::getApplication()->input->get('contact_info', '', 'STRING');
			...
			case $this->_task == 'save_css' or $this->_task == 'apply_css' :
				...
				$postData['profile_css_code'] = JFactory::getApplication()->input->get('profile_css_code', '', 'STRING');
				break;
			case $this->_task == 'save_email' or $this->_task == 'apply_email' :
				...
				$postData['mail_template'] = JFactory::getApplication()->input->get('mail_template', '', 'STRING');
				break;
			default :
				...
				$postData['required_field_mark'] = JFactory::getApplication()->input->get('required_field_mark', '', 'STRING');
	...
	function writeOtherTables( $postData = array(), $id = 0 ) {
		...
		$thank_you_message = JFactory::getApplication()->input->get('thank_you_message', '', 'STRING');
		if (strlen(trim($thank_you_message)) == 0) {
			$thank_you_message = '&nbsp;';
		}
		$thank_you_message = $this->replace_specialchars($thank_you_message);
		$required_field_notification = JFactory::getApplication()->input->get('required_field_notification', '', 'STRING');
	...
	function writeCSS( $postData = array(), $id = 0 ) {
		...
		$css_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'cssprofiles'.DIRECTORY_SEPARATOR.'profile_css_'.$id.'.css');
		$postData['profile_css_code'] = str_replace('aiContactSafe_mainbody_1 ', 'aiContactSafe_mainbody_'.$id.' ', $postData['profile_css_code']);
	...
	function writeEmail( $postData = array(), $id = 0 ) {
		...
		$mail_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates'.DIRECTORY_SEPARATOR.'mail_'.$id.'.php');
	...
	function deleteData() {
		...
		if (is_array($profiles_ids)) {
			...
			foreach($profiles_ids as $pf_id){
				$css_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'cssprofiles'.DIRECTORY_SEPARATOR.'profile_css_'.$pf_id.'.css');
				if (JFile::exists($css_file)) {
					JFile::delete($css_file);
				}
				$mail_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates'.DIRECTORY_SEPARATOR.'mail_'.$pf_id.'.php');
	...
	function readMailTemplate( $id = 0 ) {
		...
		$mail_code = file_get_contents($mail_file);
		return $mail_code;
	}
	...
	function getCSS() {
		...
		$css_type = JFactory::getApplication()->input->get('css_type', '');
		// reset the css code
		$css_code = '';
		...
		$css_file = JPATH_ROOT.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'message'.DIRECTORY_SEPARATOR.'tmpl'.DIRECTORY_SEPARATOR.'profile_'.$css_type.'.css';
		if (JFile::exists($css_file)) {
			$css_code = file_get_contents($css_file);
