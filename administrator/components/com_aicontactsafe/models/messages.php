...
class AiContactSafeModelMessages extends AiContactSafeModelDefault {
	...
	function banIP() {
		...
		$db->setQuery($query);
		$ips_to_ban = $db->loadColumn(); //load###ResultArray();
	...
	function deleteSelected() {
		// read the ids of the records seleted for deletion
		$cids = JFactory::getApplication()->input->get('cids', '', 'STRING');
		JArrayHelper::toInteger($cid);
	...
	function changeStatus() {
		...
		$cid = JFactory::getApplication()->input->get('cid', array(), 'ARRAY');
		JArrayHelper::toInteger($cid);
		$new_status = JFactory::getApplication()->input->get('new_status', 0, 'INT');
