...
class com_aicontactsafeInstallerScript {
	protected $_version = '2.0.21c.stable.j3.alfa';
	protected $extensions_updated;
	/**
...
//		$lang->load('com_aicontactsafe.sys', JPATH_ROOT . DIRECTORY_SEPARATOR . 'administrator');
		...
		$aicontactsafe_contactinformations = $db->getTableColumns('#__aicontactsafe_contactinformations');
		$aicontactsafe_fields = $db->getTableColumns('#__aicontactsafe_fields');
		$aicontactsafe_profiles =  $db->getTableColumns('#__aicontactsafe_profiles');
		$aicontactsafe_messages = $db->getTableColumns('#__aicontactsafe_messages');
		$aicontactsafe_messagefiles = $db->getTableColumns('#__aicontactsafe_messagefiles');
	...
	*/
		...
		$att_folder = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'attachments';
		...
		$css_folder = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'cssprofiles';
		if (!JFolder::exists($css_folder)) {
			JFolder::create($css_folder);
		}
		$src = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'index.html');
		$dest = JPath::clean($css_folder.DIRECTORY_SEPARATOR.'index.html');
		if (!JFile::exists($dest)) {
			JFile::copy($src, $dest);
		}
		$email_folder = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates';
		if (!JFolder::exists($email_folder)) {
			JFolder::create($email_folder);
		}
		$src = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'index.html');
		$dest = JPath::clean($email_folder.DIRECTORY_SEPARATOR.'index.html');
		if (!JFile::exists($dest)) {
			JFile::copy($src, $dest);
		}
		...
		$value = 'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'attachments';
		...
		if (strlen(trim($config_value)) == 0) {
			...
			$db->setQuery( $query );
			$db->execute();
		} else if (trim($config_value) == 'components&#92;com_aicontactsafe&#92;attachments' || trim($config_value) == 'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'attachments') {
		...
		foreach($profiles as $profile) {
			...
			$src_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'message'.DIRECTORY_SEPARATOR.'tmpl'.DIRECTORY_SEPARATOR.'profile_align_margin.css');
			$dst_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'cssprofiles'.DIRECTORY_SEPARATOR.'profile_css_'.$profile->id.'.css');
			if (!is_file($dst_file)) {
			...
			$src_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'mail'.DIRECTORY_SEPARATOR.'tmpl'.DIRECTORY_SEPARATOR.'mail.php');
			$dst_file = JPath::clean(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates'.DIRECTORY_SEPARATOR.'mail_'.$profile->id.'.php');
			if (!is_file($dst_file)) {
		...
		if (is_file($xml_file)) {
			@$installer->install(JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.'mod_aicontactsafe');
			$this->extensions_updated[] = 'mod_aicontactsafe';
		}
		if(version_compare(JVERSION, '1.6.0', 'ge')) {
		...
		if (is_file($xml_file)) {
			@$installer->install(JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.'plg_aicontactsafeform');
			$this->extensions_updated[] = 'plg_aicontactsafeform';
		}
		if(version_compare(JVERSION, '1.6.0', 'ge')) {
		...
		if (is_file($xml_file)) {
			@$installer->install(JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.'plg_aicontactsafelink');
			$this->extensions_updated[] = 'plg_aicontactsafelink';
		}
	}
	...
	function postflight($type, $parent) {
		...
		$lang = JFactory::getLanguage();
		$lang->load('com_aicontactsafe.sys', JPATH_ROOT . DIRECTORY_SEPARATOR . 'administrator');
		$parent->getParent()->set('message', JText::_($parent->get('manifest')->description));
	?>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<td width="50%" valign="top">
					...
					<?php echo JText::_('COM_AICONTACTSAFE_VERSION'); ?>&nbsp;<?php echo $parent->get('manifest')->version; ?><br/>
					<?php
						if (count($this->extensions_updated) > 0) {
							echo 'aiContactSafe extensions updated ('.implode(',',$this->extensions_updated).').<br/>';
						}
					?>
