...
class AiContactSafeViewMessages extends AiContactSafeViewDefault {
	...
	function setVariables() {
		...
		switch(true) {
			...
			case $this->_task == 'export' :
				$this->format = JFactory::getApplication()->input->get('format', '');
				if ($this->format == 'raw') {
		...
		switch(true) {
			...
			default:
				...
				if ( $this->_task == 'display' ) {
					...
					$footer .= '<table id="changeStatus" style="border-width:0; padding:0; border-spacing:2;">';
					$footer .= '<tr>';
					$footer .= '<td>'.JText::_('COM_AICONTACTSAFE_CHANGE_STATUS_TO').'</td>';
					$footer .= '<td>&nbsp;&nbsp;&nbsp;</td>';
					$footer .= '<td>'.$this->change_status.'</td>';
					$footer .= '<td>&nbsp;&nbsp;&nbsp;</td>';
					$footer .= '<td><button class="btn" onclick="javascript:if (document.adminForm.boxchecked.value == 0){ alert(\''.JText::_('COM_AICONTACTSAFE_PLEASE_MAKE_A_SELECTION_FROM_THE_LIST').'\');}else{submitbutton(\'changestatus\')}">'.JText::_('COM_AICONTACTSAFE_GO').'</button></td>';
					$footer .= '</tr>';
					$footer .= '</table>';
				}
				$footer .= JHtml::_( 'form.token' );
