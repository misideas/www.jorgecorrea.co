...
class AiContactSafeViewDefault extends JViewLegacy {
	...
	function callCssFile($cssFile = '') {
		...
		if ($use_css) {
			...
			if (strlen($cssFile) > 0) {
				...
				$tPath = JPATH_ROOT.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template_name.DIRECTORY_SEPARATOR.'html'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.$cssFile;
	...
	function setToolbar() {
		if ($this->_backend) {
			...
			JToolBarHelper::title( JText::_('COM_AICONTACTSAFE').$pageTitle,'generic_ai.png' );
