...
class AiContactSafeModelMessage extends AiContactSafeModelDefault {
	...
	function deleteUploadedFile() {
		...
		$filename = JFactory::getApplication()->input->get('filename', '', 'CMD');
		// import joomla clases to manage file system
		jimport('joomla.filesystem.file');
		// get the path to attachments upload
		$upload_folder = str_replace('\\',DIRECTORY_SEPARATOR,$this->_config_values['upload_attachments']);
		$upload_folder = str_replace('/',DIRECTORY_SEPARATOR,$upload_folder);
		$upload_folder = str_replace('&#92;',DIRECTORY_SEPARATOR,$upload_folder);
		$path_upload = JPATH_ROOT.DIRECTORY_SEPARATOR.$upload_folder;

		$file = $path_upload.DIRECTORY_SEPARATOR.$filename;
	...
	function uploadFile() {
		...
		$field = JFactory::getApplication()->input->get('field', '', 'CMD');
		$r_id = JFactory::getApplication()->input->get('r_id', 0, 'INT');
		$errorType = 0;
		$errorMessage = '';
		$attachedFileId = 0;
		...
		$file = JFactory::getApplication()->input->files->get($field);
		...
		if ( strlen(trim($file['name'])) > 0 ) {
			...
			if ( $errorType == 0 ) {
				...
				if ($newFile->store()) {
					...
					$filepath = JPath::clean($path_upload.DIRECTORY_SEPARATOR.$file_name);
	...
	function securityCheck($postData) {
		...
		foreach($fields as $field) {
			...
			if ($field->field_type == 'ED' || $field->field_type == 'HD') {
				$field_value = JFactory::getApplication()->input->get($field->name, '', 'STRING');
			...
			if ( $profile->use_captcha == 1 || ($profile->use_captcha == 2 && $this->_user_id == 0) ) {
				switch($profile->captcha_type) {
					...
					case 1:
						...
						$captchaParam = new JForm('returnType=boolean');
	...
	function getReturnLink($new_values = array(), $use_ajax = 0) {
		if ($use_ajax) {
			...
			$link = JFactory::getApplication()->input->get('return_to', '', 'STRING');
		}
	...
	function sendEmail() {
		...
		if ($profile->email_mode) {
			$body = '<table style="border-width:0; padding:0; border-spacing:2;">';
			$body .= '<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>';
		} else {
			$body = " \n\n ";
		}
		// generate the body of the message recorded into the database
		$body_recorded = '<table style="border-width:0; padding:0; border-spacing:2;">';
		...
		if ( $spam_control == 1 or $spam_control == 2 ) {
			...
			if ($profile->use_mail_template) {
				...
				$tPath = JPATH_ROOT.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template_name.DIRECTORY_SEPARATOR.'html'.DIRECTORY_SEPARATOR.'com_aicontactsafe'.DIRECTORY_SEPARATOR.'mail'.DIRECTORY_SEPARATOR.'mail_'.$profile->id.'.php';

				$nameMailTemplate = '';
				if (JFile::exists($tPath)) {
					$nameMailTemplate = $tPath;
				} else {
					$tPath = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates'.DIRECTORY_SEPARATOR.'mail_'.$profile->id.'.php';
					if (JFile::exists($tPath)) {
						$nameMailTemplate = $tPath;
					} else {
						$nameMailTemplate = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'aicontactsafe'.DIRECTORY_SEPARATOR.'mailtemplates'.DIRECTORY_SEPARATOR.'mail.php';
					}
				}
	...
	function downloadFile() {
		...
		$file = $path_upload.DIRECTORY_SEPARATOR.$file_name;
	...
	function ban_ip( $ips_list = '' ) {
		...
		if ($isOK) {
			...
			$mode = true;
			$isOK = JFactory::getMailer()->sendMail($from, $fromname, $email_recipient, $subject, $body, $mode);
		} else {
			$this->_app->_session->set( 'isOK:' . $this->_sTask, false );
	...
	function deleteFileAfterMessageSent( $message_id = 0, $postData ) {
		...
		if (count($files) > 0) {
			foreach($files as $file) {
				$delete_file = $path_upload.DIRECTORY_SEPARATOR.$file->name;
